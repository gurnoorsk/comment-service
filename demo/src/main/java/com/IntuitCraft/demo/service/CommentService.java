...
public class CommentService {
    ...
    public String deleteCommentById(Long commentId,String userId){
        ...
        if(canDelete) {
            ...
            res.add(comment.getId());
            res = findAllRelatedComments(comment,0,res);
            commentRepository.deleteCommentById(res);
            return "Comment Deleted Successfully";
        }
        throw CommentsException.UNABLE_TO_DELETE_COMMENT;
    }

    private List<Long> findAllRelatedComments(Comment comment,int page,List<Long> res){
        List<Comment> ids = new ArrayList();
        while(true){
            Page<Comment> com = commentRepository.findCommentsByParentIdPageable(comment.getId(), PageRequest.of(page, CommentsServiceConstants.FETCH_SIZE));
            ids.clear();
            ids.addAll(com.stream().toList());
            res.addAll(ids.stream().map(e -> e.getId()).collect(Collectors.toList()));
            for(Comment comm:ids){
                res = findAllRelatedComments(comm,page,res);
            }
            if(!com.hasNext()){
                break;
            }
    ...
    public void deleteCommentForPost(Long postId) {
        ...
        do{
            comments = commentRepository.findFirstLevelCommentsByPostId(postId, PageRequest.of(page, CommentsServiceConstants.FETCH_SIZE));
            if(comments.isEmpty()){
                break;
            }
            ...
            while(itr.hasNext()){
                ...
                deleteCommentById(comment.getId(),comment.getUserId());
            }
            page++;
