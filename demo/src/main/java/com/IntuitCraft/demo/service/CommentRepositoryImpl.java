...
//    }
    ...
    public Comment save(Comment comment) {
        String sql = "INSERT INTO comments (post_id, parent_id, user_id, content, likes_count, dislikes_count, date_added,time_added_epoch) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection connection = dataSource.getConnection();
            ...
            preparedStatement.setLong(8,new java.util.Date().getTime());
    ...
    @Override
    public void deleteCommentById(List<Long> commentIds){
        String sql = "DELETE FROM comments WHERE id IN (<commentIds>)";
        try (Handle handle = jdbi.open()) {
            String joinedIds = commentIds.stream()
                    .map(String::valueOf)
                    .collect(Collectors.joining(", "));


            sql = sql.replace("<commentIds>", joinedIds);
    ...
    public void updateComment(Comment comment) {
        ...
        if (comment.getParentId() != null) {
            sql.append(first ? "" : ", ").append("parent_id = :parentId");
            first = false;
        }
