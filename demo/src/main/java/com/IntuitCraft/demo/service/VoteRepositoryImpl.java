...
public class VoteRepositoryImpl implements IVoteRepository {
    ...
    public void saveLikes(Likes like) {
        ...
        jdbi.useHandle(handle -> {
            handle.createUpdate("INSERT INTO likes (comment_id, user_id, post_id, timestamp) VALUES (:commentId, :userId, :postId, :timestamp)")
                    .bind("commentId", like.getCommentId())
                    .bind("userId", like.getUserId())
                    .bind("postId", like.getPostId())
                    .bind("timestamp",like.getTimestamp())
                    .execute();
        });
    }
    ...
    public Page<String> findUserIdsForVoteByCommentId(Long commentId, String tableName, Pageable pageable) {
        String sql="select user_id from " +tableName+ " where comment_id = :commentId order by timestamp desc LIMIT :limit OFFSET :offset";
...
//        List<String> userIDsForPage = allUserIds.subList(fromIndex, toIndex);
        ...
        return new PageImpl<>(allUserIds, pageable, allUserIds.size());
    ...
    public void saveDisLikes(Dislikes dislikes) {
        ...
        jdbi.useHandle(handle -> {
            handle.createUpdate("INSERT INTO dislikes (comment_id, user_id, post_id,timestamp) VALUES (:commentId, :userId, :postId, :timestamp)")
                    .bind("commentId", dislikes.getCommentId())
                    .bind("userId", dislikes.getUserId())
                    .bind("postId", dislikes.getPostId())
                    .bind("timestamp",dislikes.getTimestamp())
                    .execute();
        });
    }
